...
//go:generate sh -c "rm -rf n2n && git clone https://github.com/ntop/n2n.git && cd n2n && git checkout 2.8-stable && sed -i -e 's@./configure@@g' ./autogen.sh && ./autogen.sh && ./configure $([ -n \"${HOST}\" ] && echo --host=\"${HOST}\") && make -j $(nproc) && ar d libn2n.a edge.o && ar d libn2n.a example_edge_embed.o && ar d libn2n.a example_edge_embed_quick_edge_init.o && ar d libn2n.a example_sn_embed.o && ar d libn2n.a sn.o"
