	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, id := range args {
			...
			go func(id string, wg *sync.WaitGroup) {
				response, err := client.Delete(ctx, &api.EdgeManagedId{
					Id: id,
				})
				if err != nil {
					log.Error(err.Error())
...
func init() {
	deleteCmd.PersistentFlags().StringVarP(&serverHostPortFlag, serverHostPortKey, "s", constants.EdgedServerHostPortDefault, constants.HostPortDocs)
